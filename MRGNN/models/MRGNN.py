...
class MRGNN(nn.Module):
    ...
    def __init__(self, n_blocks, input_size, hidden_size, n_hidden, window_size, n_sensor, dropout=0.1, model="MAF",
        ...
        self.num_register_tokens = 20
        self.register_token = nn.Parameter(torch.zeros(1, self.num_register_tokens, input_size))
        self.pos_embed_register = nn.Parameter(torch.zeros(1, self.num_register_tokens, input_size))

        self.d_model = 32
        self.pre_linear = nn.Linear(input_size,self.d_model)

        self.rnn = Mamba(d_model=self.d_model, d_state=32, d_conv=4, expand=2).to("cuda")
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.project = nn.Linear(self.d_model, self.hidden_size)
    ...
    def test(self, x, ):
        ...
        B, L, D = x.shape
        register_tokens = self.register_token.expand(B, -1, -1) + self.pos_embed_register
        insert_interval = L // (self.num_register_tokens + 1)
        new_x = []
        for i in range(L):
            if (i + 1) % insert_interval == 0 and (i // insert_interval) < self.num_register_tokens:
                new_x.append(register_tokens[:, i // insert_interval, :].unsqueeze(1))
            new_x.append(x[:, i, :].unsqueeze(1)) 
        ...
        h = h.permute(0, 2, 3, 1)
        register_output = h[:, -self.num_register_tokens:, :].mean(dim=1)
        # reshappe N*K*L,H
        h = h.reshape((-1, h.shape[3]))
        x = x.reshape((-1, full_shape[3]))

        adjusted_window_size = full_shape[2] + self.num_register_tokens
        log_prob = self.nf.log_prob(x, full_shape[1], adjusted_window_size, h).reshape([full_shape[0], -1])
